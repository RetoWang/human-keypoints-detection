...
from modules.keypoints import BODY_PARTS_KPT_IDS, BODY_PARTS_PAF_IDS, a
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1:
                ...
                if kpt_a_id in right:
                    cv2.circle(img, (int(x_a), int(y_a)), 8, Pose.color3, -1)
                if kpt_a_id in neck:
                    cv2.circle(img, (int(x_a), int(y_a)), 7, (255,200,1), -1)
                    xy = "%d,%d" % (x_a, y_a)
            ...
            if global_kpt_b_id != -1:
                ...
                if kpt_b_id in right:
                    cv2.circle(img, (int(x_b), int(y_b)), 7, Pose.color3, -1)
                if kpt_b_id in neck:
                    cv2.circle(img, (int(x_b), int(y_b)), 7, (255,200,1), -1)
                    # print("r_ear_id", kpt_a_id, ":", x_a, y_a)
                    cv2.putText(img, 'neck', (x_b + 9, y_b-19), cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,200,1), 1)
                    xy = "%d,%d" % (x_b, y_b)
                    cv2.putText(img, xy, (x_b + 9, y_b), cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,200,1), thickness=1)
                else:
            ...
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                if kpt_a_id in right or kpt_b_id in right:
                    cv2.line(img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), (0, 0, 255), 3)
                else:
